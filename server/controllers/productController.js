...
const getProductsForTag = async (req, res) => {
  ...
  try {
    const products = await pool.query(`SELECT p.product_id, p.name, p.price, p.category, d.discount_amount, p.images,p.gender
...
const getAllProducts = async (req, res) => {
  try {
    const products = await pool.query(`SELECT * FROM product_base`);
    res.json({ products: products.rows });
  } catch (err) {
    console.error(err.message);
    res.status(500).json({ error: 'Server error' });
...
const addReview = async (req, res) => {
  ...
  if (!account_id || !product_id || !rating || !comment?.trim()) {
  ...
  try {
    await pool.query(
      `
            INSERT INTO product_reviews (account_id, product_id, rating, comment)
            VALUES ($1, $2, $3, $4)
            ON CONFLICT (account_id, product_id)
            DO UPDATE SET rating = EXCLUDED.rating, comment = EXCLUDED.comment, created_at = NOW()
            `,
      [account_id, product_id, rating, comment.trim()]
    );
